...
Each trial contains a two-phase blinded packet for 90 raw/full decision-quality pairs. The frozen generation run requested no automated semantic judge and no independent reviewer responses were populated. After the run completed, a separately authorized Luna High process reviewed the 1,620 saved packet answers under a blinded, answer-only rubric. The process is an AI triage diagnostic, not an outcome authority: it has no human calibration or adjudication, Luna judges Luna candidate answers, and known position, verbosity, limited-reasoning, and self-preference risks remain \citep{zheng2023judge,wataoka2024selfpreference}. It cannot rank agronomic answer quality or promote a system.
...
\subsection{Post-hoc blinded AI triage (diagnostic only)}

Figure~\ref{fig:posthoc-triage} and Table~\ref{tab:posthoc-triage} summarize a separately authorized Luna High review of the 90 raw/full decision-quality pairs in each of nine candidate trials (1,620 saved answers). The score is a weighted internal rubric: agronomic accuracy 0.35, decision relevance 0.20, completeness/actionability 0.20, calibration/safety 0.15, and crop/region/source fit 0.10; each dimension is rated 0--4 and the weighted result is reported on 0--100. It is not combined with the deterministic endpoints, retrieval traces, or any release decision.

Operationally, all selected answers received one terminal review, and the public aggregate binds the selection manifest, authorization, and saved judgment file hashes. Scientifically, the judge is not validated: no human labels, calibration set, inter-rater study, or agronomic adjudication exist. Its self-reported high-confidence rate is 99.3\%, while repeated identical answer texts can receive different scores. The score also correlates negatively with answer length in this sample, and Luna judges outputs from the same Luna configuration. These diagnostics are reasons to treat the observed raw/full contrasts as hypotheses for independent review, not evidence that one configuration is safer, more correct, or better.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/rc3_posthoc_semantic_review.pdf}
  \caption{\textbf{Separately authorized post-hoc blinded Luna High triage.} Filled marks are case-level trial-average means; open marks are the three observed trial means; whiskers are fixed-suite case-resampling sensitivity intervals. The right panel is the matched full-system minus raw contrast. These are uncalibrated AI rubric outputs, not validated semantic-quality scores, rankings, or promotion evidence.}
  \label{fig:posthoc-triage}
\end{figure*}
...
RC3 is intentionally nonclaim. The exposed suite was inspected and used for repair. It has no real-user cases, no untouched prospective holdout, no independent agronomist judgments, no field outcomes, and no legal-compliance adjudication. The post-hoc AI triage adds an internally specified diagnostic, not a substitute: its rubric is uncalibrated, lacks reference labels and independent adjudication, and uses the Luna configuration to judge Luna outputs. Retrieval presence does not prove relevance or use. Verification activity does not prove improvement. Exact repetition does not prove correctness. Latency and token observations do not compare local energy with remote cost. Candidate configurations use different verification profiles, so cross-candidate full-system contrasts are not pure model effects.
...
The public checkpoint includes answer-free response measurements, trial inventory, summary tables, five vector figures, exact generated manuscript values, a claim-to-evidence manifest, and a self-verifying package manifest. The analysis reads only the nine completion-receipted trial databases plus the separately named post-hoc judgment artifact; it does not glob the retention root. It verifies database hashes, suite and source identities, arm and observation cardinalities, zero generation-time semantic judgments, the post-hoc selection and authorization identities, the two frozen parser defects, the retained latency outlier, and all paired keys before producing a result. Machine-local paths are not serialized.
